...
### 011: Rewire the UI as a server-rendered Jinja2 + HTMX + Alpine + Tailwind site — 2026-09-12

- **Context:** The owner was not satisfied with the UI and asked for a full rewire. The classic UI is ~230KB of hand-rolled HTML strings (`ui.py`, `console.py`), dark, with a photo hero that fills a phone's first screen.
- **Options considered:** (a) keep HTML strings and restyle; (b) Jinja2 templates + HTMX + Alpine.js + Tailwind, server-rendered; (c) a React/TypeScript front-end over a JSON API.
- **Decision:** (b), chosen by the owner. One codebase and one language for a two-person Python team, the fastest pages on a budget Android phone, and the domain modules reused unchanged. Light theme by default (read in daylight), dark available. Every asset is served from `/static/app`; the compiled CSS is committed so running the app needs no build. The new site mounts beside the classic screens and replaces them section by section.
- **Consequences / accepted risks:** A build step exists for CSS changes (`python -m vyuha_platform css`). GitHub is blocked on the founder's network, so the Tailwind CLI comes from npm and needs Node for rebuilds (not for running). Two UIs coexist until cutover (Phase 7); every new page links to the classic screen that does its job meanwhile.

### 012: Onboarding is an operator-led, eight-stage stepper — 2026-09-12

- **Context:** Onboarding was two fields and a choice of "upload" or "books". A real business arrives with history in Tally, Excel, paper registers or nowhere, and needs an opening position before any statement ties out.
- **Decision:** An Onboarding Studio for the Vyuha team: profile → data map (per domain: where the record lives, how far back, who keeps it) → masters → opening position → history import → check & sign-off → continuous feeds → people, alerts & go-live. Stage status is derived from the business wherever possible. Stages 1–3 ship in Phase 1; 4–6 with the data core in Phase 2; 7–8 with the feeds in Phase 6.
- **Consequences / accepted risks:** The data map decides how a business is read (`data_mode`) only while it holds no data, so answering it later never flips an upload business into merge mode silently. The onboarding record is JSON until the per-business SQLite store lands in Phase 2.
